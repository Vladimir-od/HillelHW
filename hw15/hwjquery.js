...
function addComment() {
    ...
    if (validateInputAndComment()) {
        text = findTagsAndLinks(text);
...
function validateInputAndComment() {
    const regexp = /^[A-Z][a-z]*(\s[A-Z][a-z]*)?(\s[A-Z][a-z]*)?$/;
...
function findTagsAndLinks(text) {
    const regexp = /((https?:\/\/|ftps?:\/\/|www\.)((?![.,?!;:()]*(\s|$))[^\s]){2,})/gm;
    const newText = text.replace(/<.*?>/gm, "").replace(regexp, '<a href="$1">$1</a>');
    return newText;
};